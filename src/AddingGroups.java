...
/*Создаём интерфейсную ссылку типа Collection и присваиваем ей новый экземпляр параметризованного ArrayList.  При этом типа нашего объекта повышается (upcasting) до интерфейса Collection.
Этот экземпляр ArrayList создаётся с аргументом в виде объекта, реализующего интерфейс List (а значит, и интерфейс Collection, т.к. интерфейс List реализует интерфейс Collection). Для этого класс ArrayList имеет специальный конструктор, способный принимать в качестве аргумента любой объект, реализующий интерфейс Collection.
В свою очередь статический метод Arrays.asList в качестве аргумента получает либо массив, либо список элементов, разделённых запятыми и возвращает объект, реализующий интерфейс List.*/
       // Единственный способ заполнить ArrayList при создании – создавать его с аргументом объектом типа Collection, т.к. у ArrayList есть для этого специальный конструктор. Остальные два конструктора предназначены для создания пустого ArrayList или ArrayList желаемой ёмкости. Однако такой способ довольно медленный.
